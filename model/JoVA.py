...
class JoVA():
    ...
    def prepare_model(self):
        ...
        self.label_U = tf.palceholder(dtype=tf.float32, shape=[None, self.num_items], name="label_U")
        self.label_I = tf.palceholder(dtype=tf.float32, shape=[None, self.num_users], name="label_I")
        ...
        x_user = Dense(intermediate_dim, use_bias=False)(Input(tensor=self.input_R_U))
        ...
        outputs_user = Dense(self.num_items, activation='softmax')(x_user_decoder)
        ...
        reconstruction_loss_u = categorical_crossentropy(Input(tensor=self.input_R_U),outputs_user)
        ...
        xItem = Dense(intermediate_dim, use_bias=False)(inp_tmp)
        ...
        outputs_item = Dense(self.num_users, activation='softmax')(xItem)
        ...
        reconstruction_loss = categorical_crossentropy(Input(tensor=inp_tmp),
                                                  outputs_item)
        ...
        for i in range(self.num_batch_U):  # iterate each batch
            ...
            for j in range(self.num_batch_I):
                ...
                if j == self.num_batch_I - 1:
                    ...
                    p_input, n_input = evaluate.pairwise_neg_sampling(self.train_R_U, row_idx, col_idx, 1)

                    input_R_U = self.train_R_U[row_idx, :]
                    input_R_I = self.train_R_I[:, col_idx]
                   ...
                   batch_cost += Cost
        batch_cost /= self.num_batch_U * self.num_batch_I
    ...
    def test_model(self, itr):
        ...
        Cost, Decoder = self.sess.run(
            ...
            feed_dict={self.input_R_U: self.train_R_U, self.input_R_I: self.train_R_I, self.input_P_cor: [[0, 0]],
